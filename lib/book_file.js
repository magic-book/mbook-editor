...
class BookRes {
   ...
   *          current tab path
   *        {Object|String} img:
   *          {Object} NativeImage or {String} local image path
   */
  * saveImage (file, img) {
    let p, self = this;
    ...
    if (typeof img != 'string') {
      ...
      var imageName = (this.resStat[pathId] + 1) + '.' + imageData.type;
      yield fsp.writeFile(
        path.join(p, imageName),
        imageData.buffer
      );
    } else {
      let matches = img.match(/\.(bmp|gif|jpeg|jpg|tif|tiff|ico)$/);
      if (matches) {
        let ext = matches[1];
        var imageName = (this.resStat[pathId] + 1) + '.' + ext;
        yield fsp.writeFile(
          path.join(p, imageName),
          yield fsp.readFile(img)
        );
      } else {
